...
import com.report.listener.EconomicIndicatorListener;
...
@RestController
@RequestMapping("/economic-indicator")
@CrossOrigin(origins = "*")
public class EconomicIndicatorController {
    ...
    @GetMapping("/page")
    public Result<Page<EconomicIndicator>> getPage(
            @RequestParam(defaultValue = "1") Integer current,
            @RequestParam(defaultValue = "10") Integer size,
            @RequestParam(required = false) String keyword) {
        
        Page<EconomicIndicator> page = new Page<>(current, size);
        QueryWrapper<EconomicIndicator> queryWrapper = new QueryWrapper<>();
        ...
        Page<EconomicIndicator> resultPage = mapper.selectPage(page, queryWrapper);
        return Result.success(resultPage);
    }

    @PostMapping
    public Result<?> save(@RequestBody EconomicIndicator entity) {
        mapper.insert(entity);
        return Result.success("新增成功", null);
    }

    @PostMapping("/update")
    public Result<String> update(@RequestBody EconomicIndicator entity) {
    ...
    public void exportData(HttpServletResponse response, @RequestParam(required = false) String keyword) throws IOException {
        setupExcelResponse(response, "经济指标导出");
        QueryWrapper<EconomicIndicator> queryWrapper = new QueryWrapper<>();
        if (StringUtils.hasText(keyword)) {
            queryWrapper.like("farm_name", keyword).or().like("farm_code", keyword);
        }
        queryWrapper.orderByDesc("created_at");
        List<EconomicIndicator> list = mapper.selectList(queryWrapper);
        EasyExcel.write(response.getOutputStream(), EconomicIndicator.class).sheet("经济指标").doWrite(list);
    }

    @GetMapping("/template")
    public void downloadTemplate(HttpServletResponse response) throws IOException {
        setupExcelResponse(response, "经济指标导入模板");
        EasyExcel.write(response.getOutputStream(), EconomicIndicator.class).sheet("模板").doWrite(new ArrayList<>());
    }
    ...
    public Result<?> importData(MultipartFile file, @RequestParam(defaultValue = "0") Integer sheetNo) {
        try {
            EasyExcel.read(file.getInputStream(), EconomicIndicator.class, new EconomicIndicatorListener(mapper)).sheet(sheetNo).doRead();
