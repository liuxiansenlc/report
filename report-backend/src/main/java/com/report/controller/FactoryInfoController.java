...
import com.report.listener.FactoryInfoListener;
...
public class FactoryInfoController {
    ...
    @GetMapping("/page")
    public Result<Page<FactoryInfo>> getPage(
            @RequestParam(defaultValue = "1") Integer current,
            @RequestParam(defaultValue = "10") Integer size,
            @RequestParam(required = false) String keyword) {
        
        Page<FactoryInfo> page = new Page<>(current, size);
        QueryWrapper<FactoryInfo> queryWrapper = new QueryWrapper<>();
        ...
        Page<FactoryInfo> resultPage = mapper.selectPage(page, queryWrapper);
        return Result.success(resultPage);
    }

    @PostMapping
    public Result<?> save(@RequestBody FactoryInfo entity) {
        mapper.insert(entity);
        return Result.success("新增成功", null);
    }

    @PostMapping("/update")
    public Result<String> update(@RequestBody FactoryInfo entity) {
    ...
    public void exportData(HttpServletResponse response, @RequestParam(required = false) String keyword) throws IOException {
        setupExcelResponse(response, "工厂基础信息导出");
        QueryWrapper<FactoryInfo> queryWrapper = new QueryWrapper<>();
        if (StringUtils.hasText(keyword)) {
            queryWrapper.like("name", keyword);
        }
        queryWrapper.orderByDesc("created_at");
        List<FactoryInfo> list = mapper.selectList(queryWrapper);
        EasyExcel.write(response.getOutputStream(), FactoryInfo.class).sheet("工厂基础信息").doWrite(list);
    }

    @GetMapping("/template")
    public void downloadTemplate(HttpServletResponse response) throws IOException {
        setupExcelResponse(response, "工厂基础信息导入模板");
        EasyExcel.write(response.getOutputStream(), FactoryInfo.class).sheet("模板").doWrite(new ArrayList<>());
    }
    ...
    public Result<?> importData(MultipartFile file, @RequestParam(defaultValue = "0") Integer sheetNo) {
        try {
            EasyExcel.read(file.getInputStream(), FactoryInfo.class, new FactoryInfoListener(mapper)).sheet(sheetNo).doRead();
