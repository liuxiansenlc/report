...
import com.report.listener.ServiceStationListener;
...
public class ServiceStationController {
    ...
    @GetMapping("/page")
    public Result<Page<ServiceStation>> getPage(
            @RequestParam(defaultValue = "1") Integer current,
            @RequestParam(defaultValue = "10") Integer size,
            @RequestParam(required = false) String keyword) {
        
        Page<ServiceStation> page = new Page<>(current, size);
        QueryWrapper<ServiceStation> queryWrapper = new QueryWrapper<>();
        ...
        Page<ServiceStation> resultPage = mapper.selectPage(page, queryWrapper);
        return Result.success(resultPage);
    }

    @PostMapping
    public Result<?> save(@RequestBody ServiceStation entity) {
        mapper.insert(entity);
        return Result.success("新增成功", null);
    }

    @PostMapping("/update")
    public Result<String> update(@RequestBody ServiceStation entity) {
    ...
    public void exportData(HttpServletResponse response, @RequestParam(required = false) String keyword) throws IOException {
        setupExcelResponse(response, "服务站基础信息导出");
        QueryWrapper<ServiceStation> queryWrapper = new QueryWrapper<>();
        if (StringUtils.hasText(keyword)) {
            queryWrapper.like("name", keyword);
        }
        queryWrapper.orderByDesc("created_at");
        List<ServiceStation> list = mapper.selectList(queryWrapper);
        EasyExcel.write(response.getOutputStream(), ServiceStation.class).sheet("服务站基础信息").doWrite(list);
    }

    @GetMapping("/template")
    public void downloadTemplate(HttpServletResponse response) throws IOException {
        setupExcelResponse(response, "服务站基础信息导入模板");
        EasyExcel.write(response.getOutputStream(), ServiceStation.class).sheet("模板").doWrite(new ArrayList<>());
    }
    ...
    public Result<?> importData(MultipartFile file, @RequestParam(defaultValue = "0") Integer sheetNo) {
        try {
            EasyExcel.read(file.getInputStream(), ServiceStation.class, new ServiceStationListener(mapper)).sheet(sheetNo).doRead();
            return Result.success("导入成功，重复名称已自动跳过！", null);
        } catch (Exception e) {
            e.printStackTrace();
