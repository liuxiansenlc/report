  ...
  <div class="module-container">
    <el-card class="modern-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="name" label="服务点名称" min-width="180" show-overflow-tooltip />
        <el-table-column prop="address" label="服务点地址" min-width="250" show-overflow-tooltip />
...
const rules = {
  name: [{ required: true, message: '请输入服务点名称', trigger: 'blur' }]
}
...
const getList = async () => {
  try {
    loading.value = true
    const res = await request.get('/service-station/page', { params: queryParams })
...
const handleBatchDelete = async () => {
  ...
  try {
    await request.post('/service-station/delete/batch', selectedIds.value)
...
const handleDelete = async (row) => {
  try {
    await request.post(`/service-station/delete/${row.id}`)
...
const submitForm = () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        if (form.id) {
          await request.post('/service-station/update', form)
          ElMessage.success('更新成功')
        } else {
          await request.post('/service-station', form)
