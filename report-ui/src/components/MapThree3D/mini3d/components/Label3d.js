...
export class Label3d {
  constructor({ scene, camera, time, sizes, canvas }) {
    ...
    this.canvas = canvas
    this.parent = null
    let { layoutWidth, layoutHeight, width, height } = this.sizes
    let css3dRender = new CSS3DRenderer() // 实例化css3d渲染器
    css3dRender.setSize(layoutWidth || width, layoutHeight || height) // 设置渲染器的尺寸
  ...
  resize() {
    let { layoutWidth, layoutHeight, width, height } = this.sizes
    this.css3dRender.setSize(layoutWidth || width, layoutHeight || height)
  }
}
